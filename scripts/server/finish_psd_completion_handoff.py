"""Finish the already authorized user-authorized complete-source retry policy after the v5 drain."""
...
def main():
    ...
    def status(phase, **extra):
        o.save(OUT/'completion-policy-handoff.json', {'phase': phase, 'time': time.time(), **extra})
    ...
    try:
        ...
        while True:
            ...
            time.sleep(2)
        status('launching_complete_source_recovery', completed=proof['completed'])
        ...
        if recovery['reused'] + recovery['retry_remaining'] != proof['completed'] or recovery['explicit_budget_extensions'] != 0:
        ...
        while time.monotonic() < deadline:
            ...
            if (space.get('measurement_status') == 'complete' and space.get('admission_open')
                ...
                status('verified_new_sampling_without_run_cap', reused=recovery['reused'], retry_remaining=recovery['retry_remaining'],
                       completed=progress['completed'], ceiling_bytes=None, originals_preserved=True)
                return
            time.sleep(10)
        raise TimeoutError('No verified new complete_source sampling before maintenance deadline')
