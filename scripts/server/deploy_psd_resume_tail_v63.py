"""Stage and launch the bounded PSD tail-resume scheduler (v63)."""
...
OLD = ROOT / "training-artifacts/psd-flash37-recovery-20260918-v62"
DEPLOY = ROOT / "training-artifacts/psd-resume-tail-20260919-v63"
CODE = DEPLOY / "code"
OVERLAYS = DEPLOY / "overlays"
CONTROL = ROOT / "runs/psd-formal-prepare-controller-20260919-resume-tail-v6"
PREVIOUS_CONTROL = ROOT / "runs/psd-formal-prepare-controller-20260918-flash37-recovery-v5"
SERVICE = ROOT / "inference/psd-sft3084-20260916"
FILES = (
    "scripts/run_psd_feedback_canary.py",
    "scripts/run_psd_repair_driver.py",
    "scripts/server/deploy_psd_resume_tail_v63.py",
    "training/ifv_training/psd_slate.py",
    "training/ifv_training/psd_slate_feedback.py",
    "training/ifv_training/psd_slate_search.py",
...
def controller():
    result = module("psd_resume_tail_v63_controller",
...
def stage():
    if CODE.exists():
        raise RuntimeError("immutable v63 snapshot already exists")
    if not (OLD / "code").is_dir():
        raise RuntimeError("v62 source snapshot is missing")
    for relative in FILES:
        if not (OVERLAYS / relative).is_file():
            raise RuntimeError("missing v63 overlay: " + relative)
    ...
    if result:
        raise RuntimeError("v63 regression tests failed")
...
def start():
    ...
    if state.get("deployment_ready_not_live") is not True:
        raise RuntimeError("v63 deployment is not validated")
    if CONTROL.exists():
        raise RuntimeError("v63 controller already exists")
    ...
    launch = controller()
    owner = module("psd_resume_tail_v63_owner",
    ...
    command = [sys.executable, "-u",
        str(CODE / "scripts/server/deploy_psd_resume_tail_v63.py"), "worker"]
    ...
    save(CONTROL / "state.json", {"phase": "launched", "external_checks": checks,
        ...
        "attempt_budget_reset": False, "time": time.time()})
