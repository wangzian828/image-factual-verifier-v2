...
TRAIN_OUT = FINAL / "formal-training-smallbank4095-v2"
...
def main():
    ...
    gate_result = gate_out / "result.json"
    if not gate_result.is_file() or load(gate_result).get("passed") is not True:
    ...
    atomic_json(state_path, {"phase": "formal_training", "ready": str(ready), "gate": str(gate_result),
        "reused_passed_gate": True})
    command = [str(ROOT / "envs/h20-qwen35-128k/bin/python"), "-u",
        str(CODE / "scripts/server/run_psd_smallbank_training.py"),
        "--ready", str(ready), "--gate", str(gate_result), "--output", str(TRAIN_OUT)]
