...
def eager_command(command, *, gpu=3, decode_only=False, no_graph=False):
    ...
    if decode_only or no_graph:
        assert '--compilation-config' not in result
        config = {'mode':0,'cudagraph_mode':'FULL_DECODE_ONLY'} if decode_only else {'mode':3,'cudagraph_mode':'NONE'}
        return [*result, '--compilation-config', json.dumps(config)]
    return [*result, '--enforce-eager']


def execute(o, gpu=3, decode_only=False, no_graph=False):
    ...
    command = eager_command(old['command'], gpu=gpu, decode_only=decode_only, no_graph=no_graph)
...
def main():
    ...
    parser.add_argument('--gpu',type=int,choices=[1,2,3],default=3)
    parser.add_argument('--decode-only',action='store_true')
    parser.add_argument('--no-graph',action='store_true')
    args = parser.parse_args()
    if args.no_graph:
        OUT = SERVICE / f'compile-no-graph-gpu{args.gpu}-v1'
    ...
    if args.mode=='execute':
        execute(o,args.gpu,args.decode_only,args.no_graph)
    else:
        eager_command(o.load(SERVICE/f'replica-{args.gpu}.json')['command'],gpu=args.gpu,decode_only=args.decode_only,no_graph=args.no_graph)
