"""One-off owned v4 drain: hold admission's du child, never an Agent request.
...
RUN = ROOT/'runs/psd-production400x8-20260917-v4'
...
def main():
    ...
    expected = ROOT/'training-artifacts/psd-recovery-compactor-20260917-v37/run_psd_production_collection.py'
    if str(expected) not in receipt['command'] or RUN.name not in receipt['command']:
        raise ValueError('Not the explicitly owned v4 collector')
    o.checked(receipt)
        ...
        while time.monotonic()-started < 3600:
            ...
            if time.monotonic() >= next_check:
                ...
                summary = {'completed': states.count('completed'), 'running': states.count('running'),
                    ...
                    'held_measurements': len(held), 'time': time.time(), 'maintenance': 'remove_user_rejected_run_cap'}
