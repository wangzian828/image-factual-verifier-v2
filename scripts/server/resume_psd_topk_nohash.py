...
DEPLOY = ROOT / "training-artifacts/psd-lightweight-recovery-20260920-v87"
CODE = DEPLOY / "code"
...
def materialize(owner, cache: Path) -> str:
    ...
    resolved = previous_resolved if reuse_previous else OUT / "resolved-targets-stat-v87"
    ...
    datums = previous_datums if reuse_previous else OUT / "datums-stat-v87"
    ...
    ready = attest(SimpleNamespace(output=OUT / "attested-stat-v87", rollout_gate=ROLLOUT_GATE,
        datums=datums / "datums.jsonl", datum_manifest=datums / "manifest.json",
        snapshot=current_snapshot(owner)))
    atomic_json(OUT / "result-stat-v87.json", {"status": "ready_for_training", "topk": 20,
    ...
    for path in [OUT / "target-shards", *(OUT / f"gpu-{index}" for index in range(4)),
                 OUT / ".resolved-targets-stat-v87-stage"]:
        if path.exists():
            shutil.rmtree(path)
    return ready["ready"]
...
def execute(parent_pid: int) -> None:
    ...
    try:
        ...
        atomic_json(OUT / "state-stat-v87.json", {"phase": "ready_for_training", "ready": ready,
