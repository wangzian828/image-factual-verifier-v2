from __future__ import annotations

import argparse
import base64
import json
import mimetypes
import time
from pathlib import Path
from typing import Any
from urllib.error import HTTPError, URLError
from urllib.parse import urlsplit
from urllib.request import Request, urlopen

from jsonschema import Draft202012Validator


def _data_url(path: Path) -> str:
    mime = mimetypes.guess_type(path.name)[0] or "image/jpeg"
    encoded = base64.b64encode(path.read_bytes()).decode("ascii")
    return f"data:{mime};base64,{encoded}"


def _request(url: str, payload: dict[str, Any] | None = None) -> Any:
    request = Request(
        url,
        data=json.dumps(payload).encode("utf-8") if payload is not None else None,
        headers={"Content-Type": "application/json", "Authorization": "Bearer none"},
    )
    try:
        with urlopen(request, timeout=600) as response:
            raw = response.read()
    except HTTPError as exc:
        detail = exc.read().decode("utf-8", errors="replace")
        raise RuntimeError(f"HTTP {exc.code} from {url}: {detail}") from exc
    except URLError as exc:
        raise RuntimeError(f"request failed for {url}: {exc}") from exc
    if not raw:
        return None
    return json.loads(raw)


def _completion(base_url: str, payload: dict[str, Any]) -> dict[str, Any]:
    result = _request(f"{base_url}/chat/completions", payload)
    choices = result.get("choices") or []
    if len(choices) != 1:
        raise RuntimeError(f"expected one completion choice, got {len(choices)}")
    message = choices[0].get("message")
    if not isinstance(message, dict):
        raise RuntimeError("completion did not contain an assistant message")
    return {
        "message": message,
        "finish_reason": choices[0].get("finish_reason"),
        "usage": result.get("usage") or {},
    }


def _reasoning(message: dict[str, Any], gate: str) -> str:
    value = message.get("reasoning") or message.get("reasoning_content")
    if not isinstance(value, str) or not value.strip():
        raise RuntimeError(f"{gate}: reasoning was not separated or was empty")
    return value


def _content(message: dict[str, Any], gate: str) -> str:
    value = message.get("content")
    if not isinstance(value, str) or not value.strip():
        raise RuntimeError(f"{gate}: final content was empty")
    if "<think>" in value or "</think>" in value:
        raise RuntimeError(f"{gate}: reasoning leaked into final content")
    return value


def _structured(
    base_url: str,
    model: str,
    *,
    name: str,
    prompt: str,
    schema: dict[str, Any],
    max_tokens: int,
) -> dict[str, Any]:
    completion = _completion(
        base_url,
        {
            "model": model,
            "temperature": 0,
            "max_tokens": max_tokens,
            "messages": [{"role": "user", "content": prompt}],
            "response_format": {
                "type": "json_schema",
                "json_schema": {"name": name, "strict": True, "schema": schema},
            },
        },
    )
    message = completion["message"]
    reasoning = _reasoning(message, name)
    try:
        parsed = json.loads(_content(message, name))
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"{name}: final content is not JSON: {exc}") from exc
    Draft202012Validator(schema).validate(parsed)
    return {
        "output": parsed,
        "reasoning_chars": len(reasoning),
        "finish_reason": completion["finish_reason"],
        "usage": completion["usage"],
    }


def _validate_planning_semantics(value: dict[str, Any]) -> None:
    claims = value["target_facts"]
    hypotheses = value["search_hypotheses"]
    claim_keys = [item["claim_key"] for item in claims]
    hypothesis_keys = [item["hypothesis_key"] for item in hypotheses]
    if len(claim_keys) != len(set(claim_keys)):
        raise RuntimeError("image_account_planning: claim keys are not unique")
    if len(hypothesis_keys) != len(set(hypothesis_keys)):
        raise RuntimeError("image_account_planning: hypothesis keys are not unique")
    if not any(item["salience"] == "high" for item in claims):
        raise RuntimeError("image_account_planning: no high-salience claim")
    executable_first_hops = {
        "reverse_image_search",
        "text_search",
        "check_consistency",
        "analyze_visual_anomalies",
        "crop_and_inspect",
        "ocr_with_position",
    }
    if any(
        not executable_first_hops.intersection(item["suggested_tools"])
        for item in hypotheses
    ):
        raise RuntimeError("image_account_planning: hypothesis lacks a first-hop tool")


def _origin(base_url: str) -> str:
    split = urlsplit(base_url)
    return f"{split.scheme}://{split.netloc}"


def run(args: argparse.Namespace) -> dict[str, Any]:
    if not args.image.is_file():
        raise RuntimeError(f"probe image does not exist: {args.image}")
    if args.rounds < 1 or args.rounds > 8:
        raise RuntimeError("--rounds must be between 1 and 8")
    base_url = args.base_url.rstrip("/")
    origin = _origin(base_url)

    _request(f"{origin}/health")
    models = _request(f"{base_url}/models")
    cards = models.get("data") or []
    if not cards:
        raise RuntimeError("/v1/models returned no models")
    model = args.model or cards[0]["id"]
    card = next((item for item in cards if item.get("id") == model), None)
    if card is None:
        raise RuntimeError(f"served model is absent from /v1/models: {model}")
    if card.get("max_model_len") != args.expected_context:
        raise RuntimeError(
            f"expected max_model_len={args.expected_context}, got {card.get('max_model_len')}"
        )
    tokenizer_info = _request(f"{origin}/tokenizer_info")
    if not tokenizer_info.get("chat_template"):
        raise RuntimeError("/tokenizer_info did not expose the active chat template")

    text = _completion(
        base_url,
        {
            "model": model,
            "temperature": 0,
            "max_tokens": args.max_tokens,
            "messages": [{"role": "user", "content": "Reply with READY."}],
        },
    )
    text_reasoning = _reasoning(text["message"], "text")
    text_content = _content(text["message"], "text")

    image = _completion(
        base_url,
        {
            "model": model,
            "temperature": 0,
            "max_tokens": args.max_tokens,
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": "State one directly visible property."},
                        {"type": "image_url", "image_url": {"url": _data_url(args.image)}},
                    ],
                }
            ],
        },
    )
    image_reasoning = _reasoning(image["message"], "image")
    image_content = _content(image["message"], "image")

    simple_schema = {
        "type": "object",
        "properties": {
            "status": {"type": "string", "enum": ["ready"]},
            "count": {"type": "integer", "const": 1},
        },
        "required": ["status", "count"],
        "additionalProperties": False,
    }
    simple = _structured(
        base_url,
        model,
        name="reasoning_then_schema",
        prompt="Think about the request, then return status ready and count 1.",
        schema=simple_schema,
        max_tokens=args.max_tokens,
    )

    planning_schema = json.loads(args.planning_schema.read_text(encoding="utf-8"))
    planning = _structured(
        base_url,
        model,
        name="image_account_planning",
        prompt=(
            "Create a minimal image fact-check planning object. Use one high-salience "
            "relation claim anchored by fact_1 and one text_search hypothesis."
        ),
        schema=planning_schema,
        max_tokens=args.planning_max_tokens,
    )
    _validate_planning_semantics(planning["output"])

    tools = [
        {
            "type": "function",
            "function": {
                "name": "inspect",
                "description": "Inspect one requested property.",
                "parameters": {
                    "type": "object",
                    "properties": {"property": {"type": "string"}},
                    "required": ["property"],
                    "additionalProperties": False,
                },
            },
        }
    ]
    round_results = []
    for index in range(args.rounds):
        messages: list[dict[str, Any]] = [
            {"role": "system", "content": "Use the available tool."},
            {
                "role": "user",
                "content": json.dumps(
                    {"task": "inspect", "round": index + 1, "completed_rounds": index}
                ),
            },
        ]
        call = _completion(
            base_url,
            {
                "model": model,
                "temperature": 0,
                "max_tokens": args.max_tokens,
                "messages": messages,
                "tools": tools,
                "tool_choice": "required",
            },
        )
        call_message = call["message"]
        call_reasoning = _reasoning(call_message, f"tool round {index + 1}")
        calls = call_message.get("tool_calls") or []
        if len(calls) != 1:
            raise RuntimeError(f"tool round {index + 1}: expected one call, got {len(calls)}")
        tool_call = calls[0]
        function = tool_call.get("function") or {}
        if not tool_call.get("id") or function.get("name") != "inspect":
            raise RuntimeError(f"tool round {index + 1}: malformed tool call {tool_call}")
        json.loads(function.get("arguments") or "{}")
        assistant_turn = {
            "role": "assistant",
            "content": call_message.get("content"),
            "tool_calls": calls,
        }
        messages.extend(
            [
                assistant_turn,
                {
                    "role": "tool",
                    "tool_call_id": tool_call["id"],
                    "content": json.dumps({"status": "success", "round": index + 1}),
                },
                {"role": "user", "content": "Confirm the tool result in one sentence."},
            ]
        )
        continuation = _completion(
            base_url,
            {
                "model": model,
                "temperature": 0,
                "max_tokens": args.max_tokens,
                "messages": messages,
                "tools": tools,
                "tool_choice": "none",
            },
        )
        continuation_reasoning = _reasoning(
            continuation["message"], f"continuation round {index + 1}"
        )
        continuation_content = _content(
            continuation["message"], f"continuation round {index + 1}"
        )
        round_results.append(
            {
                "round": index + 1,
                "call_reasoning_chars": len(call_reasoning),
                "continuation_reasoning_chars": len(continuation_reasoning),
                "continuation": continuation_content,
                "call_usage": call["usage"],
                "continuation_usage": continuation["usage"],
            }
        )

    return {
        "schema_version": "ifv-vllm-qwen3vl-endpoint-gates-v1",
        "created_at_epoch": int(time.time()),
        "passed": True,
        "model": model,
        "max_model_len": card["max_model_len"],
        "tokenizer_class": tokenizer_info.get("tokenizer_class"),
        "gates": {
            "health": True,
            "models": True,
            "context_configuration": True,
            "text": {"content": text_content, "reasoning_chars": len(text_reasoning)},
            "image": {"content": image_content, "reasoning_chars": len(image_reasoning)},
            "reasoning_then_schema": simple,
            "image_account_planning_schema": planning,
            "independent_tool_roundtrips": round_results,
        },
    }


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--base-url", default="http://127.0.0.1:8901/v1")
    parser.add_argument("--model", default="ifv-qwen3-vl-8b-thinking-vllm")
    parser.add_argument("--image", type=Path, required=True)
    parser.add_argument(
        "--planning-schema",
        type=Path,
        default=Path(__file__).resolve().parents[2]
        / "configs"
        / "serve"
        / "image-account-planning.schema.json",
    )
    parser.add_argument("--expected-context", type=int, default=131072)
    parser.add_argument("--rounds", type=int, default=8)
    parser.add_argument("--max-tokens", type=int, default=4096)
    parser.add_argument("--planning-max-tokens", type=int, default=16384)
    parser.add_argument("--output", type=Path)
    args = parser.parse_args()
    result = run(args)
    rendered = json.dumps(result, ensure_ascii=False, indent=2)
    if args.output:
        args.output.parent.mkdir(parents=True, exist_ok=True)
        args.output.write_text(rendered + "\n", encoding="utf-8")
    print(rendered)


if __name__ == "__main__":
    main()
