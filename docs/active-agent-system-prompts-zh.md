...
- 每个 query 都要回答一个关于图片主体、事件、关系、数值、地点、日期、文字或
  可见细节的具体问题。
- 如果已有具体的人物、事件、地点、物体或图片文字，且需要据此寻找相关图片或
  含图页面，使用 `text_image_search`。它是文字到图片的搜索，不会上传当前图片；
  返回的图片和页面只是未验证候选，后续要用 `visit` 或
  `compare_with_reference` 检查。
- 如果要上传当前图片寻找 Lens 或语义图像对应，使用 `reverse_image_search`。
  任何搜索结果都不能仅因标题、主体或场景相似就当作事实证明。
