...
3. `[ ]` 使用目标 Qwen processor 重新编码正式包并确认 `passed=true`。
4. `[ ]` 下载一条完整 reasoning SFT 样本到本地人工复核。
...
9. `[ ]` 更新 Agent、Prompt、SFT、Qwen、轨迹与运维文档。
10. `[ ]` 完整本地测试、Git 提交并推送。
11. `[ ]` gpu-13 fast-forward 后重复结构审计和真实 processor 验证。
