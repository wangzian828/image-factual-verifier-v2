...
| 2026-08-31 | 真实 smoke 暴露并修复 Gemini follow-up 与跨 action session 问题 | `src/orchestrator/stage_runner.py`, `src/orchestrator/pipeline.py`, `docs/gemini-interaction-sequence.md` | 候选图不再放入 `function_result`；先下载、校验并压缩为 inline JPEG，与原图合并到同一个 `user_input`。统一 ReAct action 不再复用 provider-side `InteractionSession`，每轮由 reducer 状态重建紧凑请求。 | 本地 454/454、服务器 454/454；3.7 smoke 10/10 成功、0 工程错误；strict audit 已确认旧 session 链接为审计失败根因，待本次修复后复测。 | `922d9d2` / 待提交 |
