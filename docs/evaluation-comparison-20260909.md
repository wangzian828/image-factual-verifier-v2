...
以下五组均使用 Gemini 3.7 Flash、`thinking_level=low`、`max_output_tokens=32768` 和 v3 全材料输入。每组均有 1,526 条唯一、状态为 `completed` 的审核记录；不可运行的 1 条样本不从 1,527 报告分母中删除。
...
截至 2026-09-14，以上 Batch judge 均已完成且不应重复提交。当前仍有两项尚未进入最终 judge：

1. 新版 4,872 条训练集 SFT（`SFT-1028`）仍在生成 1,526 条 Agent 轨迹。轨迹成功补齐后再以完全相同的 v3 口径提交，并在本表增加最终行。
2. Gemini 3.1 Pro Preview Agent 的首轮全量运行只成功 389/1,526 条，另外 1,137 条全部因当时服务器 `No space left on device` 结束；这不是模型质量结果，也未提交最终 judge。必须仅重跑这 1,137 条并补齐到 1,526 个唯一成功 case 后，才能提交同口径 v3 Batch judge。原始失败记录位于服务器 `/volume/ybo/wza/runs/eval/gemini31pro-agent-formal1527-20260913/full-r2-c4`。
...
- 新版 `SFT-1028` 的最终二分类指标与 SESR 尚未产生，不使用运行中的部分样本替代最终结果。
