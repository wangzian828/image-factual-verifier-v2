...
| 6 | Gemini 3.1 Pro Agent | 62.42 | 73.61 | 64.17 | 29.71 | 95.13 | 32.68 |
...
复算主表：`python scripts/rebuild_comparison_metrics.py`；需要核对原 20 组历史记录时加 `--include-history`。展示过滤不会删除整数计数。脚本只读聚合计数，不启动模型、训练或付费 judge；修改主表时需同步计数记录，回归测试会检查两张主表与复算结果一致。

<details>
<summary>历史审核与运行记录（不进入精简主表）</summary>
