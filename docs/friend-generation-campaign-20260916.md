...
用户明确要求恢复16:06为PSD外部额度让路而排空暂停的Pro组；PSD仍使用Flash，不自动暂停朋友Pro。

- 恢复前166结构成功、54失败、216未尝试。Flash3.6已经436/436结构成功。
- 首次恢复发现冻结输入清单的JSON列表与内存元组直接比较，误报输入变化；没有重新发送该次请求。恢复包装器仅将比较对象规范化为JSON类型，并重新核对全部样本图片绑定及prompt绑定。原冻结控制器、prompt、模型、工具和预算未修改。
- `scripts/server/resume_friend_pro_campaign.py`的v2部署启动PID1404167；实际`state.json`为`running`，继续wave00的sample0221，仍串行1。新wire已有两次200及后续请求，不能将这些API成功提前计为样本完成。
- 原成功结果保留，先做未尝试项，再按原campaign的三轮补跑规则处理失败。原receipt与失败恢复日志也保留；运行状态应以实时文件为准。
