...
- 现有 `visit_many` 页面并发、共享 OCR backend 和 OCR perception cache 保持启用。
...
- EasyOCR reader 共享、Baidu token 缓存与 OCR 调用计时；
...
- EasyOCR 以服务器 CPU 运行方式为基准；Baidu API 作为显式远程 backend；
  不保留静默 fallback。
