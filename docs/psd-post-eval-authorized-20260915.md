...
## 2026-09-16 16:52 用户改为当前任务持续排查

用户明确不使用定时检查，要求一直处理到 PSD 能完整启动。原定时任务已 PAUSED，不另建定时器，也不自动恢复每小时。
当前现场排查、真实 NaN 流式复现和单卡执行模式对照见 [最新实况](psd-observed-position-correction-20260916.md)。正式 PSD 仍未开始；不以进程存活或单次成功代替验收。

## 2026-09-16 16:28 持续推进到正式启动（当前）

用户要求一直监控到 PSD 可以完整启动。原任务已暂改每 15 分钟，实际优化连续正常推进后恢复每小时；不是只监控进程存活。
v14 原控制器已结束。完整调查的审核超时已用原缓存补完，审核因提示泄漏具体答案而拒绝；该失败不重判挑结果。
现仅对该案例以原 6/12 预算继续提示修订，见 [恢复、当前控制入口与未通过门槛](psd-observed-position-correction-20260916.md)。另两例 NaN/Qwen 超时待独立诊断，不盲重跑。
正式优化仍 0 step、400×8 未开始；原三轮 SFT 参数与 fullstate 不动。原冻结 Agent、训练预算、所有审核/精确 token/多图/9B 更新及原生恢复门槛不放宽。
