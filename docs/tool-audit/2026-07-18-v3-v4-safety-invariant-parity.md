...
| Terminal verdict matches retained observations | Current raw-history Judgment basis | Binary `real`/`fake` is grounded only in successful observations retained in the canonical trace | `test_raw_history_audit.py` strict-audit test |
