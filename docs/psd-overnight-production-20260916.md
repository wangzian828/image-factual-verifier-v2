...
- 3200 条轨迹后处理已改为一次读取、16 进程流式执行，约 9 分钟完成 3200/3200；候选构建同样改为
  16 进程流式执行，不再把数十 GiB 轨迹装入父进程，得到 2199 条 repair seed、971 条
  preservation seed、30 条 source-review abstention。对应代码为 `c6d9bbb`、`50e089d`。
- 最长失败轨迹选择也已移除 64 GiB 重扫和 2.8 GiB 全量候选驻留，16 进程完成 2199→383 个任务的
  确定性选择；代码 `882b97a`，服务器 518 项 PSD 回归通过。正式修复仍按每任务最多 6 次完整重跑，
  没有缩短思考、删除图片、删工具步骤或降低方法预算。
- 首次正式 repair 暴露控制器没有继承私有外部工具环境，80 条仅基础设施失败后立即停止；没有生成可
  接受训练目标。`df73280` 增加了 Gemini、Serper、Jina、百度 OCR、OSS 五项强制预检，并从权限 0600
  的既有 runtime env 只加载白名单字段。Gemini 3.1 Pro Preview 的真实 structured probe 已通过，
  Serper 指纹与用户指定替换 key 一致；密钥未写日志或提交。
- repair 已在同一冻结 SFT3 权重、同一 383 任务选择及原目录上续跑。16 路 Gemini 实测会快速触发
  大量 HTTP 错误（50 条），因此停止该次调用且不接纳其失败为训练数据；当前控制器 v8 改为 8 路，
  case pool 仍为 40，所有传输失败保留并自动续跑。四卡 vLLM 与真实计算 guard 均保持，空闲卡会在
  30 秒低利用窗口后产生真实推理脉冲，防止资源回收。
- repair 后的 preservation/repair 组装、target 构造和 datum 构造已全部改为逐行流式写出，避免再次
  在后段把多 GiB JSONL 全量读入内存；代码 `a8ee56c`，服务器全套 518 项 PSD 回归通过，当前正式
  进程已经切入该版本。原 SFT3 权重、3200 条源轨迹和已完成修复缓存均未改动。

## 2026-09-17 17:01 全量采集通过，正式prepare已自动接续

- 正式400×8采集已完成3200/3200，0 normal error、0 pending infrastructure；最后一条首attempt的
  单次model request按1200秒网关预算报`model_request_timeout`，自动进入attempt-002并成功，证明尾部
  有界重试实际生效。最终collection gate为400 cases/3200 episodes/每题8条、selection_by_success=false，
  已通过。四卡服务、真实计算guard、compactor均保留，未因尾部样本重启或改变采样身份。
...
- 固定两条非逐字缓存案例都已真实完成quote-only修复并通过全绑定验收；第二条首次真实请求收到HTTP429，
  原失败目录保留，prefetch排空后在新目录的第1次有界重试成功。两条均各只增加一次成功quote请求，
  判决本身没有重抽或改变。
- 已启动CODE46自动接续控制器（本地提交`037e6f8`，远端PID初值1369138）：它只从当前可信reviewer
  ...
  17:00 prefetch已排空：2136 fail、974 pass、30 unresolved、60 pending transport/format尾项，valid
