...
> 2026-09-15 后续更新：当前先执行 **400 图训练 pilot**，不是全量 4,000 图采集。400 图已落盘，新分组/多位置修复入口和耗时实测见 [PSD 调整与 Agent 耗时调查](psd-adjustments-and-agent-latency-20260915.md)。下文 4,000 图池、历史验收和原准备命令保留溯源；第 7 步的全量规模不再是当前启动计划。新采样和 PSD 训练尚未启动。
