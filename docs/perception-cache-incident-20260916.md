...
`scripts/server/audit_perception_cache_identity.py`逐个核对冻结选中trace SHA、实际图片SHA和原始工具结果SHA。只将“不同图片收到同一结果且存在cache hit”计作跨图缓存组，不将同图重复使用或两个独立无缓存结果恰好相同算作缓存错配。
...
这些数量是受缓存身份缺陷影响的证据，不等于所有case最终标签错误，也没有证明每条轨迹同样程度受影响。原BAcc仍是对原输出正确计数，但不能单独解释为排除了工具错误的模型能力。其余Agent尚未审计，不能宣称安全；不将这一故障泛化到不走工具的Direct QA。
